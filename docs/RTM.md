...
| FR-ID | Requirement (summary) | Design Ref | Implementation | Unit | Component | E2E | Manual |
|-------|-----------------------|------------|----------------|------|-----------|-----|--------|
| FR-1301 | Accept mail at Port > Mail, incl. cargo-space reservation | DD §2 `MailPanel`, DD §1.1 `obligations.mail_containers` | `MailPanel.vue`, `ship.js:acceptMailContract`/`mailContainerTonsUsed`, `worker/src/routes/ships.js:accept-mail`, `d1/004_obligations.sql` (kind='mail') | — | — | — | — |
...
| NFR-6 | WCAG 2.2 AA target; keyboard shortcuts + focus traps | Manual testing; `useFocusTrap.js` review; Lighthouse accessibility audit — 100/100 on the production build as of 2026-07-13 (fixed from 97 by repairing the skip-link target) |
| NFR-6a | Keyboard-operable, visible focus | MTS-15 (manual). Sortable `MarketTable.vue` headers and its clickable rows are now keyboard-operable (`tabindex`, `@keydown.enter.space`); primary flows (Login, Passengers/Freight/Services forms, Referee panel tabs) verified. **Partial** — not yet audited for every dialog/table row action app-wide |
| NFR-6b | Color not the sole information channel | MTS-15 (manual). **Closed 2026-07-13** — `MarketTable.vue` price cells now show a ▼/▲ symbol + sr-only label alongside color; world-list travel-zone entries show a literal A/R badge + sr-only label; `ReportsPanel.vue`'s Ledger/Trades/Income-Net rows and `RouteAnalysis.vue`'s profit column were found to silently drop the +/− sign via `Math.abs()` for negative values (color-only in practice) and were fixed to always show an explicit sign. `CargoHold.vue`'s profit/loss and the sale-result flash were reviewed and already carried a sign or the words "profit"/"loss" — no change needed there |
| NFR-6c | WCAG 2.2 AA contrast ratios | MTS-15 (manual) + computed relative-luminance verification for the `dark-imperium` theme's redesigned charcoal palette (`src/lib/themes-builtin.js`), including the new `--accent-text` token (added because a plain white-on-accent-dim button text color failed 4.5:1 after the charcoal repaint — Lighthouse confirmed 100/100 accessibility after the fix). Other two built-in themes (`light-merchant`, `sepia-ancients`) reverified unaffected |
