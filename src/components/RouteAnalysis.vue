...
function projectedProfit(destWorld) {
  const snapshots = generateWorldSnapshot({
    ...
    activeEvents: [],
    // Previously omitted, which silently defaulted to CT7's own goods
    // table/pricing engine for every campaign — a real bug, not just an
    // estimate-precision gap, since it matched cargo dies against a
    // different, unrelated goods table for non-CT7 rulesets.
...
const projections = computed(() => {
  ...
  for (const w of map.worlds) {
    ...
    const totalProfit = hasCargo ? projectedProfit(w) : 0
